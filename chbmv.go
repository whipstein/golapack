...
//
//  =====================================================================
func Chbmv(uplo *byte, n *int, k *int, alpha *complex64, a *[]complex64, aoff, lda *int, x *[]complex64, xoff, incx *int, beta *complex64, y *[]complex64, yoff, incy *int) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				...
				for i = maxint(1, j-(*k)); i <= j-1; i++ {
					...
					temp2 = temp2 + conjc64((*a)[l+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[i-1+(*xoff)]
				}
			...
			for j = 1; j <= (*n); j++ {
				...
				for i = maxint(1, j-(*k)); i <= j-1; i++ {
					...
					temp2 = temp2 + conjc64((*a)[l+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[ix-1+(*xoff)]
					ix = ix + (*incx)
					iy = iy + (*incy)
				}
