...
//
//  =====================================================================
func Ztpsv(uplo *byte, trans *byte, diag *byte, n *int, ap *[]complex128, apoff *int, x *[]complex128, xoff, incx *int) {
			...
			if (*incx) == 1 {
				for j = 1; j <= (*n); j++ {
					...
					if noconj {
						...
						if nounit {
							temp = temp / conjc128((*ap)[kk+j-1-1+(*apoff)])
						}
					}
					(*x)[j-1+(*xoff)] = temp
					kk = kk + j
			...
			if (*incx) == 1 {
				for j = (*n); j >= 1; j-- {
					...
					if noconj {
						...
						if nounit {
							temp = temp / conjc128((*ap)[kk-(*n)+j-1+(*apoff)])
						}
					}
					(*x)[j-1+(*xoff)] = temp
					kk = kk - ((*n) - j + 1)
