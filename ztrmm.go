...
//
//  =====================================================================
func Ztrmm(side *byte, uplo *byte, transa *byte, diag *byte, m *int, n *int, alpha *complex128, a *[]complex128, aoff, lda *int, b *[]complex128, boff, ldb *int) {
				...
				for k = 1; k <= (*n); k++ {
					for j = 1; j <= k-1; j++ {
						if (*a)[j-1+(k-1)*(*lda)+(*aoff)] != zero {
							if noconj {
								...
								temp = (*alpha) * conjc128((*a)[j-1+(k-1)*(*lda)+(*aoff)])
							}
							for i = 1; i <= (*m); i++ {
								(*b)[i-1+(j-1)*(*ldb)+(*boff)] = (*b)[i-1+(j-1)*(*ldb)+(*boff)] + temp*(*b)[i-1+(k-1)*(*ldb)+(*boff)]
							}
					...
					if nounit {
						if noconj {
							...
							temp = temp * conjc128((*a)[k-1+(k-1)*(*lda)+(*aoff)])
						}
					}
					if temp != one {
						for i = 1; i <= (*m); i++ {
