...
//
//  =====================================================================
func Zgbmv(trans *byte, m *int, n *int, kl *int, ku *int, alpha *complex128, a *[]complex128, aoff, lda *int, x *[]complex128, xoff, incx *int, beta *complex128, y *[]complex128, yoff, incy *int) {
	var noconj bool
	var temp complex128
		...
		if (*incx) == 1 {
			for j = 1; j <= (*n); j++ {
				...
				if noconj {
					...
					for i = maxint(1, j-(*ku)); i <= minint(*m, j+(*kl)); i++ {
						temp = temp + conjc128((*a)[k+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[i-1+(*xoff)]
					}
				}
			...
			for j = 1; j <= (*n); j++ {
				...
				if noconj {
					...
					for i = maxint(1, j-(*ku)); i <= minint(*m, j+(*kl)); i++ {
						temp = temp + conjc128((*a)[k+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[ix-1+(*xoff)]
						ix = ix + (*incx)
					}
				}
				(*y)[jy-1+(*yoff)] = (*y)[jy-1+(*yoff)] + (*alpha)*temp
