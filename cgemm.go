...
//
//  =====================================================================
func Cgemm(transa *byte, transb *byte, m *int, n *int, k *int, alpha *complex64, a *[]complex64, aoff, lda *int, b *[]complex64, boff, ldb *int, beta *complex64, c *[]complex64, coff, ldc *int) {
	var conja, conjb, nota, notb bool
	var one, temp, zero complex64
	...
	if notb {
		if nota {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + conjc64((*a)[l-1+(i-1)*(*lda)+(*aoff)])*(*b)[l-1+(j-1)*(*ldb)+(*boff)]
					}
					if (*beta) == zero {
		...
		if conjb {
			...
			for j = 1; j <= (*n); j++ {
				...
				for l = 1; l <= (*k); l++ {
					temp = (*alpha) * conjc64((*b)[j-1+(l-1)*(*ldb)+(*boff)])
					for i = 1; i <= (*m); i++ {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = (*c)[i-1+(j-1)*(*ldc)+(*coff)] + temp*(*a)[i-1+(l-1)*(*lda)+(*aoff)]
					}
				}
		...
		if conjb {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + conjc64((*a)[l-1+(i-1)*(*lda)+(*aoff)])*conjc64((*b)[j-1+(l-1)*(*ldb)+(*boff)])
					}
					if (*beta) == zero {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = (*alpha) * temp
					} else {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + conjc64((*a)[l-1+(i-1)*(*lda)+(*aoff)])*(*b)[j-1+(l-1)*(*ldb)+(*boff)]
					}
					if (*beta) == zero {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = (*alpha) * temp
					} else {
		...
		if conjb {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + (*a)[l-1+(i-1)*(*lda)+(*aoff)]*conjc64((*b)[j-1+(l-1)*(*ldb)+(*boff)])
					}
					if (*beta) == zero {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = (*alpha) * temp
					} else {
