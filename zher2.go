...
//
//  =====================================================================
func Zher2(uplo *byte, n *int, alpha *complex128, x *[]complex128, xoff, incx *int, y *[]complex128, yoff, incy *int, a *[]complex128, aoff, lda *int) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				if ((*x)[j-1+(*xoff)] != zero) || ((*y)[j-1+(*yoff)] != zero) {
					temp1 = (*alpha) * conjc128((*y)[j-1+(*yoff)])
					temp2 = conjc128((*alpha) * (*x)[j-1+(*xoff)])
					for i = 1; i <= j-1; i++ {
			...
			for j = 1; j <= (*n); j++ {
				if ((*x)[jx-1+(*xoff)] != zero) || ((*y)[jy-1+(*yoff)] != zero) {
					temp1 = (*alpha) * conjc128((*y)[jy-1+(*yoff)])
					temp2 = conjc128((*alpha) * (*x)[jx-1+(*xoff)])
					ix = kx
					iy = ky
					for i = 1; i <= j-1; i++ {
