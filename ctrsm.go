...
//
//  =====================================================================
func Ctrsm(side *byte, uplo *byte, transa *byte, diag *byte, m *int, n *int, alpha *complex64, a *[]complex64, aoff, lda *int, b *[]complex64, boff, ldb *int) {
				...
				for k = (*n); k >= 1; k-- {
					if nounit {
						if noconj {
							...
							temp = one / conjc64((*a)[k-1+(k-1)*(*lda)+(*aoff)])
						}
						for i = 1; i <= (*m); i++ {
					...
					for j = 1; j <= k-1; j++ {
						if (*a)[j-1+(k-1)*(*lda)+(*aoff)] != zero {
							if noconj {
								...
								temp = conjc64((*a)[j-1+(k-1)*(*lda)+(*aoff)])
							}
							for i = 1; i <= (*m); i++ {
