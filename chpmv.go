...
//
//  =====================================================================
func Chpmv(uplo *byte, n *int, alpha *complex64, ap *[]complex64, apoff *int, x *[]complex64, xoff, incx *int, beta *complex64, y *[]complex64, yoff, incy *int) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 = temp2 + conjc64((*ap)[k-1+(*apoff)])*(*x)[i-1+(*xoff)]
					k = k + 1
				}
			...
			for j = 1; j <= (*n); j++ {
				...
				for k = kk; k <= kk+j-2; k++ {
					...
					temp2 = temp2 + conjc64((*ap)[k-1+(*apoff)])*(*x)[ix-1+(*xoff)]
					ix = ix + (*incx)
					iy = iy + (*incy)
				}
