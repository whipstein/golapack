...
//
//  =====================================================================
func Zhemv(uplo *byte, n *int, alpha *complex128, a *[]complex128, aoff, lda *int, x *[]complex128, xoff, incx *int, beta *complex128, y *[]complex128, yoff, incy *int) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 = temp2 + conjc128((*a)[i-1+(j-1)*(*lda)+(*aoff)])*(*x)[i-1+(*xoff)]
				}
			...
			for j = 1; j <= (*n); j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 = temp2 + conjc128((*a)[i-1+(j-1)*(*lda)+(*aoff)])*(*x)[ix-1+(*xoff)]
					ix = ix + (*incx)
					iy = iy + (*incy)
				}
