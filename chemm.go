...
//
//  =====================================================================
func Chemm(side *byte, uplo *byte, m *int, n *int, alpha *complex64, a *[]complex64, aoff, lda *int, b *[]complex64, boff, ldb *int, beta *complex64, c *[]complex64, coff, ldc *int) {
	var upper bool
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					...
					for k = 1; k <= i-1; k++ {
						...
						temp2 = temp2 + (*b)[k-1+(j-1)*(*ldb)+(*boff)]*conjc64((*a)[k-1+(i-1)*(*lda)+(*aoff)])
					}
					if (*beta) == zero {
		...
		for j = 1; j <= (*n); j++ {
			...
			for k = 1; k <= j-1; k++ {
				if upper {
					...
					temp1 = (*alpha) * conjc64((*a)[j-1+(k-1)*(*lda)+(*aoff)])
				}
				for i = 1; i <= (*m); i++ {
