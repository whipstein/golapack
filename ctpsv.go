...
//
//  =====================================================================
func Ctpsv(uplo *byte, trans *byte, diag *byte, n *int, ap *[]complex64, apoff *int, x *[]complex64, xoff, incx *int) {
			...
			if (*incx) == 1 {
				for j = 1; j <= (*n); j++ {
					...
					if noconj {
						...
						for i = 1; i <= j-1; i++ {
							temp = temp - conjc64((*ap)[k-1+(*apoff)])*(*x)[i-1+(*xoff)]
							k = k + 1
						}
						if nounit {
							temp = temp / conjc64((*ap)[kk+j-1-1+(*apoff)])
						}
					}
					(*x)[j-1+(*xoff)] = temp
					kk = kk + j
				...
				for j = 1; j <= (*n); j++ {
					...
					if noconj {
						...
						for k = kk; k <= kk+j-2; k++ {
							temp = temp - conjc64((*ap)[k-1+(*apoff)])*(*x)[ix-1+(*xoff)]
							ix = ix + (*incx)
						}
						if nounit {
							temp = temp / conjc64((*ap)[kk+j-1-1+(*apoff)])
			...
			if (*incx) == 1 {
				for j = (*n); j >= 1; j-- {
					...
					if noconj {
						...
						if nounit {
							temp = temp / conjc64((*ap)[kk-(*n)+j-1+(*apoff)])
						}
					}
					(*x)[j-1+(*xoff)] = temp
					kk = kk - ((*n) - j + 1)
