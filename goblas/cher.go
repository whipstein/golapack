...
//
//  =====================================================================
func Cher(major, uplo *byte, n *int, alpha *float32, x *[]complex64, incx *int, a *[][]complex64, lda *int) {
	var temp complex64
	...
	if Lsame(uplo, func() *byte { y := byte('U'); return &y }()) {
		...
		if *incx == 1 {
			for j = 1; j <= *n; j++ {
				if (*x)[j-1] != 0.0 {
					...
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0) + complex(real((*x)[j-1]*temp), 0.0)
				} else {
			...
			for j = 1; j <= *n; j++ {
				if (*x)[jx-1] != 0.0 {
					...
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0) + complex(real((*x)[jx-1]*temp), 0.0)
				} else {
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0)
				}
				jx += *incx
		...
		if *incx == 1 {
			for j = 1; j <= *n; j++ {
				if (*x)[j-1] != 0.0 {
					temp = complex(*alpha, 0.0) * complex64(cmplx.Conj(complex128((*x)[j-1])))
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0) + complex(real(temp*(*x)[j-1]), 0.0)
					for i = j + 1; i <= *n; i++ {
						(*a)[i-1][j-1] += (*x)[i-1] * temp
					}
				} else {
			...
			for j = 1; j <= *n; j++ {
				if (*x)[jx-1] != 0.0 {
					temp = complex(*alpha, 0.0) * complex64(cmplx.Conj(complex128((*x)[jx-1])))
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0) + complex(real(temp*(*x)[jx-1]), 0.0)
					ix = jx
					for i = j + 1; i <= *n; i++ {
						ix += *incx
						(*a)[i-1][j-1] += (*x)[ix-1] * temp
