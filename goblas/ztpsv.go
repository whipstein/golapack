...
//
//  =====================================================================
func Ztpsv(major, uplo, trans, diag *byte, n *int, ap *[]complex128, x *[]complex128, incx *int) {
	var temp complex128
	...
	if info != 0 {
		Xerbla(func() *string { y := "Ztpsv"; return &y }(), &info)
		return
	}
	//
			...
			if *incx == 1 {
				for j = 1; j <= *n; j++ {
					...
					if noconj {
						...
						for i = 1; i <= j-1; i++ {
							temp -= cmplx.Conj((*ap)[k-1]) * (*x)[i-1]
							k++
						}
						if nounit {
							temp = temp / cmplx.Conj((*ap)[kk+j-2])
						}
					}
					(*x)[j-1] = temp
					kk += j
			...
			if *incx == 1 {
				for j = *n; j >= 1; j-- {
					...
					if noconj {
						...
						if nounit {
							temp = temp / cmplx.Conj((*ap)[kk-(*n)+j-1])
						}
					}
					(*x)[j-1] = temp
					kk -= ((*n) - j + 1)
