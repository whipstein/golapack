...
//
//  =====================================================================
func Zhemm(major, side, uplo *byte, m, n *int, alpha *complex128, a *[][]complex128, lda *int, b *[][]complex128, ldb *int, beta *complex128, c *[][]complex128, ldc *int) {
	...
	if info != 0 {
		Xerbla(func() *string { y := "Zhemm"; return &y }(), &info)
		return
	}
	//
			...
			for j = 1; j <= *n; j++ {
				for i = 1; i <= *m; i++ {
					...
					for k = 1; k <= i-1; k++ {
						...
						temp2 += (*b)[k-1][j-1] * cmplx.Conj((*a)[k-1][i-1])
					}
					if *beta == 0.0 {
		...
		for j = 1; j <= *n; j++ {
			...
			for k = 1; k <= j-1; k++ {
				if upper {
					...
					temp1 = (*alpha) * cmplx.Conj((*a)[j-1][k-1])
				}
				for i = 1; i <= *m; i++ {
