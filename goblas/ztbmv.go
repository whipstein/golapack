...
//
//  =====================================================================
func Ztbmv(major, uplo, trans, diag *byte, n, k *int, a *[][]complex128, lda *int, x *[]complex128, incx *int) {
	var temp complex128
	...
	if info != 0 {
		Xerbla(func() *string { y := "Ztbmv"; return &y }(), &info)
		return
	}
	//
			...
			if *incx == 1 {
				for j = *n; j >= 1; j-- {
					...
					if noconj {
						...
						if nounit {
							temp = temp * cmplx.Conj((*a)[kplus1-1][j-1])
						}
						for i = j - 1; i >= max(1, j-(*k)); i-- {
							temp += cmplx.Conj((*a)[l+i-1][j-1]) * (*x)[i-1]
						}
					}
					(*x)[j-1] = temp
				}
				...
				for j = *n; j >= 1; j-- {
					...
					if noconj {
						...
						for i = j - 1; i >= max(1, j-(*k)); i-- {
							temp += cmplx.Conj((*a)[l+i-1][j-1]) * (*x)[ix-1]
							ix -= *incx
						}
					}
					(*x)[jx-1] = temp
