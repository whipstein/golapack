...
//
//  =====================================================================
func Cgerc(major *byte, m, n *int, alpha *complex64, x *[]complex64, incx *int, y *[]complex64, incy *int, a *[][]complex64, lda *int) {
	var temp complex64
	...
	if info != 0 {
		Xerbla(func() *string { y := "Cgerc"; return &y }(), &info)
		return
	}
	//
	...
	if *incx == 1 {
		for j = 1; j <= *n; j++ {
			if (*y)[jy-1] != 0.0 {
				temp = (*alpha) * complex64(cmplx.Conj(complex128((*y)[jy-1])))
				for i = 1; i <= *m; i++ {
