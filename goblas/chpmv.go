...
//
//  =====================================================================
func Chpmv(uplo *byte, n *int, alpha *complex128, ap *[]complex128, x *[]complex128, incx *int, beta *complex128, y *[]complex128, incy *int) {
	one := new(complex128)
	zero := new(complex128)
	...
	if Lsame(uplo, func() *byte{y := byte('u'); return &y }()) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for (*j) = 1; (*j) <= (*n); (*j)++ {
				...
				(*y)[(*j)-(1)] = (*y)[(*j)-(1)] + (*temp1)*real(((*ap)[(*kk)+(*j)-0])) + (*alpha)*(*temp2)
				(*kk) = (*kk) + (*j)
			//Label60:
			}
		} else {
			...
			for (*j) = 1; (*j) <= (*n); (*j)++ {
				...
				(*y)[(*jy)-(1)] = (*y)[(*jy)-(1)] + (*temp1)*real(((*ap)[(*kk)+(*j)-0])) + (*alpha)*(*temp2)
				(*jx) = (*jx) + (*incx)
				(*jy) = (*jy) + (*incy)
				(*kk) = (*kk) + (*j)
			//Label80:
