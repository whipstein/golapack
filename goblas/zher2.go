...
//
//  =====================================================================
func Zher2(major, uplo *byte, n *int, alpha *complex128, x *[]complex128, incx *int, y *[]complex128, incy *int, a *[][]complex128, lda *int) {
	...
	if info != 0 {
		Xerbla(func() *string { y := "Zher2"; return &y }(), &info)
		return
	}
	//
		...
		if *incx == 1 && *incy == 1 {
			for j = 1; j <= *n; j++ {
				if (*x)[j-1] != 0.0 || (*y)[j-1] != 0.0 {
					...
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1])+real((*x)[j-1]*temp1+(*y)[j-1]*temp2), 0.0)
				} else {
			...
			for j = 1; j <= *n; j++ {
				if (*x)[jx-1] != 0.0 || (*y)[jy-1] != 0.0 {
					...
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1])+real((*x)[jx-1]*temp1+(*y)[jy-1]*temp2), 0.0)
				} else {
					(*a)[j-1][j-1] = complex(real((*a)[j-1][j-1]), 0.0)
				}
				jx += *incx
