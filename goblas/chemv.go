...
//
//  =====================================================================
func Chemv(major, uplo *byte, n *int, alpha *complex64, a *[][]complex64, lda *int, x *[]complex64, incx *int, beta *complex64, y *[]complex64, incy *int) {
	...
	if Lsame(uplo, func() *byte { y := byte('U'); return &y }()) {
		...
		if (*incx == 1) && (*incy == 1) {
			for j = 1; j <= *n; j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 += complex64(cmplx.Conj(complex128((*a)[i-1][j-1]))) * (*x)[i-1]
				}
			...
			for j = 1; j <= *n; j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 += complex64(cmplx.Conj(complex128((*a)[i-1][j-1]))) * (*x)[ix-1]
					ix += *incx
					iy += *incy
				}
		...
		if (*incx == 1) && (*incy == 1) {
			for j = 1; j <= *n; j++ {
				temp1 = (*alpha) * (*x)[j-1]
				temp2 = 0.0
				(*y)[j-1] = (*y)[j-1] + temp1*complex(real((*a)[j-1][j-1]), 0.0)
				for i = j + 1; i <= *n; i++ {
