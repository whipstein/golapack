...
//
//  =====================================================================
func Ztrmm(major, side, uplo, transa, diag *byte, m, n *int, alpha *complex128, a *[][]complex128, lda *int, b *[][]complex128, ldb *int) {
	var temp complex128
	...
	if info != 0 {
		Xerbla(func() *string { y := "Ztrmm"; return &y }(), &info)
		return
	}
	//
				...
				for k = 1; k <= *n; k++ {
					for j = 1; j <= k-1; j++ {
						if (*a)[j-1][k-1] != 0.0 {
							if noconj {
								...
								temp = (*alpha) * cmplx.Conj((*a)[j-1][k-1])
							}
							for i = 1; i <= *m; i++ {
								(*b)[i-1][j-1] += temp * (*b)[i-1][k-1]
							}
					...
					if nounit {
						if noconj {
							...
							temp = temp * cmplx.Conj((*a)[k-1][k-1])
						}
					}
					if temp != 1.0 {
						for i = 1; i <= *m; i++ {
