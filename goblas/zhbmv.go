...
//
//  =====================================================================
func Zhbmv(major, uplo *byte, n, k *int, alpha *complex128, a *[][]complex128, lda *int, x *[]complex128, incx *int, beta *complex128, y *[]complex128, incy *int) {
	...
	if Lsame(uplo, func() *byte { y := byte('U'); return &y }()) {
		...
		if *incx == 1 && *incy == 1 {
			for j = 1; j <= *n; j++ {
				...
				for i = max(1, j-(*k)); i <= j-1; i++ {
					...
					temp2 += cmplx.Conj((*a)[l+i-1][j-1]) * (*x)[i-1]
				}
			...
			for j = 1; j <= *n; j++ {
				...
				for i = max(1, j-(*k)); i <= j-1; i++ {
					...
					temp2 += cmplx.Conj((*a)[l+i-1][j-1]) * (*x)[ix-1]
					ix += *incx
					iy += *incy
				}
		...
		if *incx == 1 && *incy == 1 {
			for j = 1; j <= *n; j++ {
				temp1 = (*alpha) * (*x)[j-1]
				temp2 = 0.0
				(*y)[j-1] += temp1 * complex(real((*a)[0][j-1]), 0.0)
				l = 1 - j
