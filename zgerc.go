...
//
//  =====================================================================
func Zgerc(m *int, n *int, alpha *complex128, x *[]complex128, xoff, incx *int, y *[]complex128, yoff, incy *int, a *[]complex128, aoff, lda *int) {
	var temp complex128
	...
	if (*incx) == 1 {
		for j = 1; j <= (*n); j++ {
			if (*y)[jy-1+(*yoff)] != zero {
				temp = (*alpha) * conjc128((*y)[jy-1+(*yoff)])
				for i = 1; i <= (*m); i++ {
