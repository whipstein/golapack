...
//
//  =====================================================================
func Zhemm(side *byte, uplo *byte, m *int, n *int, alpha *complex128, a *[]complex128, aoff, lda *int, b *[]complex128, boff, ldb *int, beta *complex128, c *[]complex128, coff, ldc *int) {
	var upper bool
	...
	if Lsame(side, func() *byte { y := byte('L'); return &y }()) {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					...
					for k = 1; k <= i-1; k++ {
						...
						temp2 = temp2 + (*b)[k-1+(j-1)*(*ldb)+(*boff)]*conjc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])
					}
					if (*beta) == zero {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = temp1*complex(real((*a)[i-1+(i-1)*(*lda)+(*aoff)]), 0.0) + (*alpha)*temp2
					} else {
						(*c)[i-1+(j-1)*(*ldc)+(*coff)] = (*beta)*(*c)[i-1+(j-1)*(*ldc)+(*coff)] + temp1*complex(real((*a)[i-1+(i-1)*(*lda)+(*aoff)]), 0.0) + (*alpha)*temp2
					}
				}
			}
		} else {
		...
		//
		for j = 1; j <= (*n); j++ {
			temp1 = (*alpha) * complex(real((*a)[j-1+(j-1)*(*lda)+(*aoff)]), 0.0)
			if (*beta) == zero {
				for i = 1; i <= (*m); i++ {
			...
			for k = 1; k <= j-1; k++ {
				if upper {
					...
					temp1 = (*alpha) * conjc128((*a)[j-1+(k-1)*(*lda)+(*aoff)])
				}
				for i = 1; i <= (*m); i++ {
