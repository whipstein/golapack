...
//
//  =====================================================================
func Zhpmv(uplo *byte, n *int, alpha *complex128, ap *[]complex128, apoff *int, x *[]complex128, xoff, incx *int, beta *complex128, y *[]complex128, yoff, incy *int) {
	...
	if Lsame(uplo, func() *byte { y := byte('U'); return &y }()) {
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				...
				for i = 1; i <= j-1; i++ {
					...
					temp2 = temp2 + conjc128((*ap)[k-1+(*apoff)])*(*x)[i-1+(*xoff)]
					k = k + 1
				}
				(*y)[j-1+(*yoff)] = (*y)[j-1+(*yoff)] + temp1*complex(real((*ap)[kk+j-1-1+(*apoff)]), 0.0) + (*alpha)*temp2
				kk = kk + j
			}
		} else {
			jx = kx
			...
			for j = 1; j <= (*n); j++ {
				...
				for k = kk; k <= kk+j-2; k++ {
					...
					temp2 = temp2 + conjc128((*ap)[k-1+(*apoff)])*(*x)[ix-1+(*xoff)]
					ix = ix + (*incx)
					iy = iy + (*incy)
				}
				(*y)[jy-1+(*yoff)] = (*y)[jy-1+(*yoff)] + temp1*complex(real((*ap)[kk+j-1-1+(*apoff)]), 0.0) + (*alpha)*temp2
				jx = jx + (*incx)
				jy = jy + (*incy)
				kk = kk + j
			}
		...
		if ((*incx) == 1) && ((*incy) == 1) {
			for j = 1; j <= (*n); j++ {
				temp1 = (*alpha) * (*x)[j-1+(*xoff)]
				temp2 = zero
				(*y)[j-1+(*yoff)] = (*y)[j-1+(*yoff)] + temp1*complex(real((*ap)[kk-1+(*apoff)]), 0.0)
				k = kk + 1
				for i = j + 1; i <= (*n); i++ {
					(*y)[i-1+(*yoff)] = (*y)[i-1+(*yoff)] + temp1*(*ap)[k-1+(*apoff)]
					temp2 = temp2 + conjc128((*ap)[k-1+(*apoff)])*(*x)[i-1+(*xoff)]
			...
			for j = 1; j <= (*n); j++ {
				temp1 = (*alpha) * (*x)[jx-1+(*xoff)]
				temp2 = zero
				(*y)[jy-1+(*yoff)] = (*y)[jy-1+(*yoff)] + temp1*complex(real((*ap)[kk-1+(*apoff)]), 0.0)
				ix = jx
				iy = jy
