...
func TestZblasLevel1(t *testing.T) {
	...
	for _, sname := range snames {
		if sname == "Dznrm2" || sname == "Dzasum" || sname == "Zscal" || sname == "Zdscal" || sname == "Izamax" {
			var i, len, nc, np1 int
			var sa float64 = 0.3
			var sb float64 = 0.0
			var ca complex128 = complex(0.4, -0.7)
			var cb complex128 = complex(0, 0)
			cx := make([]complex128, 8+50)
			mwpcs := make([]complex128, 5)
			mwpct := make([]complex128, 5)
			strue2 := make([]float64, 5)
			strue4 := make([]float64, 5)
			itrue3 := make([]int, 5)
			ctrue5 := make([]complex128, 8*5*2)
			ctrue6 := make([]complex128, 8*5*2)
			cv := make([]complex128, 8*5*2)
			...
			for off = 0; off <= 25; off += 5 {
				for incx = 1; incx <= 2; incx++ {
					for np1 = 1; np1 <= 5; np1++ {
						...
						if sname == "Dznrm2" {
							dcompare1(Dznrm2(&n, &cx, &off, &incx), strue2[np1-1], &strue2, np1-1, sfac, t, sname)
						} else if sname == "Dzasum" {
							dcompare1(Dzasum(&n, &cx, &off, &incx), strue4[np1-1], &strue4, np1-1, sfac, t, sname)
						} else if sname == "Zscal" {
							Zscal(&n, &ca, &cx, &off, &incx)
							zcompare(len, &cx, off, &ctrue5, 0+(np1-1+(incx-1)*5)*8, &ctrue5, 0+(np1-1+(incx-1)*5)*8, sfac, t, sname)
							for i = 1; i <= 5; i++ {
								mwpct[i-1] = (0.0 + 0.0*1i)
								mwpcs[i-1] = (1.0 + 1.0*1i)
							}
							Zscal(&ifive, &cb, &cx, &off, &incx)
							zcompare(5, &cx, off, &mwpct, 0, &mwpcs, 0, sfac, t, sname)
						} else if sname == "Zdscal" {
							Zdscal(&n, &sa, &cx, &off, &incx)
							zcompare(len, &cx, off, &ctrue6, 0+(np1-1+(incx-1)*5)*8, &ctrue6, 0+(np1-1+(incx-1)*5)*8, sfac, t, sname)
							sb = 0.0
							for i = 1; i <= 5; i++ {
								mwpct[i-1] = (0.0 + 0.0*1i)
								mwpcs[i-1] = (1.0 + 1.0*1i)
							}
							Zdscal(&ifive, &sb, &cx, &off, &incx)
							zcompare(5, &cx, off, &mwpct, 0, &mwpcs, 0, sfac, t, sname)
							...
							Zdscal(&ifive, &sb, &cx, &off, &incx)
							zcompare(5, &cx, off, &mwpct, 0, &mwpcs, 0, sfac, t, sname)
						} else if sname == "Izamax" {
							icompare1(Izamax(&n, &cx, &off, &incx), itrue3[np1-1], t, sname)
						}
					}
				}
			}
		...
		} else if sname == "Zdotc" || sname == "Zdotu" || sname == "Zaxpy" || sname == "Zcopy" || sname == "Zswap" {
			var i, ki, kn, ksize, lenx, leny, mx, my, nc int
			var ca complex128 = complex(0.4, -0.7)
			cdot := make([]complex128, 1)
			csize1 := make([]complex128, 4)
			csize3 := make([]complex128, 14)
			cx := make([]complex128, 7+50)
			cx1 := make([]complex128, 7)
			cy := make([]complex128, 7+50)
			cy1 := make([]complex128, 7)
			incxs := make([]int, 4)
			incys := make([]int, 4)
			ns := make([]int, 4)
			csize2 := make([]complex128, 7*2)
			ct6 := make([]complex128, 4*4)
			ct7 := make([]complex128, 4*4)
			lens := make([]int, 4*2)
			ct10x := make([]complex128, 7*4*4)
			ct10y := make([]complex128, 7*4*4)
			ct8 := make([]complex128, 7*4*4)
			...
			for off = 0; off <= 25; off += 5 {
				for ki = 1; ki <= 4; ki++ {
					...
					for kn = 1; kn <= 4; kn++ {
						...
						if sname == "Zdotc" {
							cdot[0] = Zdotc(&n, &cx, &off, &incx, &cy, &off, &incy)
							zcompare(1, &cdot, 0, &ct6, kn-1+(ki-1)*4, &csize1, kn-1, sfac, t, sname)
						} else if sname == "Zdotu" {
							cdot[0] = Zdotu(&n, &cx, &off, &incx, &cy, &off, &incy)
							zcompare(1, &cdot, 0, &ct7, kn-1+(ki-1)*4, &csize1, kn-1, sfac, t, sname)
						} else if sname == "Zaxpy" {
							Zaxpy(&n, &ca, &cx, &off, &incx, &cy, &off, &incy)
							zcompare(leny, &cy, off, &ct8, 0+(kn-1+(ki-1)*4)*7, &csize2, 0+(ksize-1)*7, sfac, t, sname)
						} else if sname == "Zcopy" {
							Zcopy(&n, &cx, &off, &incx, &cy, &off, &incy)
							zcompare(leny, &cy, off, &ct10y, 0+(kn-1+(ki-1)*4)*7, &csize2, 0, 1.0, t, sname)
						} else if sname == "Zswap" {
							Zswap(&n, &cx, &off, &incx, &cy, &off, &incy)
							zcompare(lenx, &cx, off, &ct10x, 0+(kn-1+(ki-1)*4)*7, &csize3, 0, 1.0, t, sname)
							zcompare(leny, &cy, off, &ct10y, 0+(kn-1+(ki-1)*4)*7, &csize3, 0, 1.0, t, sname)
						}
					}
				}
			}
...
func TestZblasLevel2(t *testing.T) {
	...
	var alpha, als, beta, bls, transl complex128
	var err, errmax, ralpha, rals float64
	var i, j, n int
	var ok bool = true
	var idim []int = []int{0, 1, 2, 3, 5, 9}
	var kb []int = []int{0, 1, 2, 4}
	var inc []int = []int{1, 2, -1, -2}
	var alf []complex128 = []complex128{complex(0.0, 0.0), complex(1.0, 0.0), complex(0.7, -0.9)}
	var bet []complex128 = []complex128{complex(0.0, 0.0), complex(1.0, 0.0), complex(1.3, -1.1)}
	var ralf []float64 = []float64{0.0, 1.0, 0.7}
	var thresh float64 = 16.0
	var eps float64 = epsilonf64()
	var rzero float64 = 0.0
	var zero complex128 = complex(0.0, 0.0)
	var half complex128 = complex(0.5, 0.0)
	var one complex128 = complex(1.0, 0.0)
	var nmax int = 65
	var incmax int = 2
	var inegone int = -1
	var izero int = 0
	var ione int = 1
	var itwo int = 2
	var snames []string = []string{"ZGEMV", "ZGBMV", "ZHEMV", "ZHBMV", "ZHPMV", "ZTRMV", "ZTBMV", "ZTPMV", "ZTRSV", "ZTBSV", "ZTPSV", "ZGERC", "ZGERU", "ZHER", "ZHPR", "ZHER2", "ZHPR2"}
	isame := make([]bool, 13)
	var ichd []byte = []byte{'U', 'N'}
	var icht []byte = []byte{'N', 'T', 'C'}
	var ichu []byte = []byte{'U', 'L'}
	a := make([]complex128, nmax*nmax)
	aa := make([]complex128, nmax*nmax)
	as := make([]complex128, nmax*nmax)
	g := make([]float64, nmax)
	x := make([]complex128, nmax)
	xs := make([]complex128, nmax*incmax)
	xt := make([]complex128, nmax)
	xx := make([]complex128, nmax*incmax)
	y := make([]complex128, nmax)
	ys := make([]complex128, nmax*incmax)
	yt := make([]complex128, nmax)
	yy := make([]complex128, nmax*incmax)
	z := make([]complex128, 2*nmax)

	n = minint(int(32), nmax)
	for j = 1; j <= n; j++ {
		for i = 1; i <= n; i++ {
			a[i-1+(j-1)*nmax] = complex(float64(maxint(i-j+1, 0)), 0.0)
		}
		x[j-1] = complex(float64(j), 0.0)
		y[j-1] = zero
	}
	for j = 1; j <= n; j++ {
		yy[j-1] = complex(float64(j*((j+1)*j))/2-float64(((j+1)*j*(j-1)))/3, 0.0)
	}
	//     YY holds the exact result. On exit from SMVCH YT holds
	//     the result computed by SMVCH.
	trans = 'N'
	zmvch(trans, &n, &n, &one, &a, &izero, &nmax, &x, &izero, &ione, &zero, &y, &izero, &ione, &yt, &g, &yy, &izero, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
	same = lze(&yy, &yt, &izero, &n)
	if !same || err != rzero {
		t.Errorf(" ERROR IN ZMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALUATED WRONGLY.\n ZMVCH WAS CALLED WITH TRANS = %c AND RETURNED SAME =  %t AND ERR = %12.3f.\n THIS MAY BE DUE TO FAULTS IN THE ARITHMETIC OR THE COMPILER.\n ******* TESTS ABANDONED *******\n", trans, same, err)
	}
	trans = 'T'
	zmvch(trans, &n, &n, &one, &a, &izero, &nmax, &x, &izero, &inegone, &zero, &y, &izero, &inegone, &yt, &g, &yy, &izero, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
	same = lze(&yy, &yt, &izero, &n)
	if !same || err != rzero {
		t.Errorf(" ERROR IN ZMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALUATED WRONGLY.\n ZMVCH WAS CALLED WITH TRANS = %c AND RETURNED SAME =  %t AND ERR = %12.3f.\n THIS MAY BE DUE TO FAULTS IN THE ARITHMETIC OR THE COMPILER.\n ******* TESTS ABANDONED *******\n", trans, same, err)
	}
						...
						for _, n = range idim {
							...
							for im = 1; im <= 2; im++ {
								...
								for _, ku = range kb {
									...
									//
									transl = zero
									zmakeL2(sname[1:3], ' ', ' ', &m, &n, &a, &nmax, &aa, &aoff, &lda, &kl, &ku, &reset, &transl)
									...
									for _, trans = range []byte{'N', 'T', 'C'} {
										...
										for _, incx = range inc {
											...
											//
											transl = half
											zmakeL2("GE", ' ', ' ', &ione, &nl, &x, &ione, &xx, &xoff, func() *int { y := absint(incx); return &y }(), &izero, func() *int { y := nl - 1; return &y }(), &reset, &transl)
											if nl > 1 {
												x[nl/2-1+xoff] = zero
											...
											for _, incy = range inc {
												...
												for _, alpha = range alf {
													...
													for _, beta = range bet {
														...
														//
														transl = zero
														zmakeL2("GE", ' ', ' ', &ione, &ml, &y, &ione, &yy, &yoff, func() *int { y := absint(incy); return &y }(), &izero, func() *int { y := ml - 1; return &y }(), &reset, &transl)
														...
														//
														if full {
															Zgemv(&trans, &m, &n, &alpha, &aa, &aoff, &lda, &xx, &xoff, &incx, &beta, &yy, &yoff, &incy)
														} else if banded {
															Zgbmv(&trans, &m, &n, &kl, &ku, &alpha, &aa, &aoff, &lda, &xx, &xoff, &incx, &beta, &yy, &yoff, &incy)
														}
														//
														...
														if full {
															isame[3] = als == alpha
															isame[4] = lze(&as, &aa, &aoff, &laa)
															isame[5] = ldas == lda
															isame[6] = lze(&xs, &xx, &xoff, &lx)
															isame[7] = incxs == incx
															isame[8] = bls == beta
															if null {
																isame[9] = lze(&ys, &yy, &yoff, &ly)
															} else {
																isame[9] = lzeres("GE", ' ', &ione, &ml, &ys, &yy, &yoff, func() *int { y := absint(incy); return &y }())
															}
															isame[10] = incys == incy
														} else if banded {
															isame[3] = kls == kl
															isame[4] = kus == ku
															isame[5] = als == alpha
															isame[6] = lze(&as, &aa, &aoff, &laa)
															isame[7] = ldas == lda
															isame[8] = lze(&xs, &xx, &xoff, &lx)
															isame[9] = incxs == incx
															isame[10] = bls == beta
															if null {
																isame[11] = lze(&ys, &yy, &yoff, &ly)
															} else {
																isame[11] = lzeres("GE", ' ', &ione, &ml, &ys, &yy, &yoff, func() *int { y := absint(incy); return &y }())
															}
															isame[12] = incys == incy
														}
														//
														...
														if !null {
															...
															//                                Check the result.
															//
															zmvch(trans, &m, &n, &alpha, &a, &aoff, &nmax, &x, &xoff, &incx, &beta, &y, &yoff, &incy, &yt, &g, &yy, &yoff, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
						...
						for _, n = range idim {
							...
							for _, k = range kb {
								...
								for _, uplo = range ichu {
									...
									//
									transl = zero
									zmakeL2(sname[1:3], uplo, ' ', &n, &n, &a, &nmax, &aa, &aoff, &lda, &k, &k, &reset, &transl)
									//
									for _, incx = range inc {
										lx = absint(incx) * n
										//
										...
										//
										transl = half
										zmakeL2("GE", ' ', ' ', &ione, &n, &x, &ione, &xx, &xoff, func() *int { y := absint(incx); return &y }(), &izero, func() *int { y := n - 1; return &y }(), &reset, &transl)
										if n > 1 {
											x[n/2-1+xoff] = zero
										...
										for _, incy = range inc {
											...
											for _, alpha = range alf {
												...
												for _, beta = range bet {
													...
													//
													transl = zero
													zmakeL2("GE", ' ', ' ', &ione, &n, &y, &ione, &yy, &yoff, func() *int { y := absint(incy); return &y }(), &izero, func() *int { y := n - 1; return &y }(), &reset, &transl)
													//
													nc++
													//
													//                          Save every datum before calling the
													...
													//
													if full {
														Zhemv(&uplo, &n, &alpha, &aa, &aoff, &lda, &xx, &xoff, &incx, &beta, &yy, &yoff, &incy)
													} else if banded {
														Zhbmv(&uplo, &n, &k, &alpha, &aa, &aoff, &lda, &xx, &xoff, &incx, &beta, &yy, &yoff, &incy)
													} else if packed {
														Zhpmv(&uplo, &n, &alpha, &aa, &aoff, &xx, &xoff, &incx, &beta, &yy, &yoff, &incy)
													}
													//
													//                          Check if error-exit was taken incorrectly.
													//
													...
													if full {
														isame[2] = als == alpha
														isame[3] = lze(&as, &aa, &aoff, &laa)
														isame[4] = ldas == lda
														isame[5] = lze(&xs, &xx, &xoff, &lx)
														isame[6] = incxs == incx
														isame[7] = bls == beta
														if null {
															isame[8] = lze(&ys, &yy, &yoff, &ly)
														} else {
															isame[8] = lzeres("GE", ' ', &ione, &n, &ys, &yy, &yoff, func() *int { y := absint(incy); return &y }())
														}
														isame[9] = incys == incy
													} else if banded {
														isame[2] = ks == k
														...
														if null {
															...
															isame[9] = lzeres("GE", ' ', &ione, &n, &ys, &yy, &yoff, func() *int { y := absint(incy); return &y }())
														}
														isame[10] = incys == incy
													} else if packed {
														isame[2] = als == alpha
														isame[3] = lze(&as, &aa, &aoff, &laa)
														isame[4] = lze(&xs, &xx, &xoff, &lx)
														isame[5] = incxs == incx
														isame[6] = bls == beta
														if null {
															isame[7] = lze(&ys, &yy, &yoff, &ly)
														} else {
															isame[7] = lzeres("GE", ' ', &ione, &n, &ys, &yy, &yoff, func() *int { y := absint(incy); return &y }())
														}
														isame[8] = incys == incy
													}
													//
													...
													if !null {
														...
														//                             Check the result.
														//
														zmvch('N', &n, &n, &alpha, &a, &aoff, &nmax, &x, &xoff, &incx, &beta, &y, &yoff, &incy, &yt, &g, &yy, &yoff, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
														errmax = maxf64(errmax, err)
														//                             If got really bad answer, report and
														//                             return.
														if fatal {
							...
							for _, uplo = range ichu {
								...
								for _, trans = range icht {
									...
									for _, diag = range ichd {
										...
										//
										transl = zero
										zmakeL2(sname[1:3], uplo, diag, &n, &n, &a, &nmax, &aa, &aoff, &lda, &k, &k, &reset, &transl)
										...
										for _, incx = range inc {
											...
											if sname[3:5] == "MV" {
												if full {
													Ztrmv(&uplo, &trans, &diag, &n, &aa, &aoff, &lda, &xx, &xoff, &incx)
												} else if banded {
													Ztbmv(&uplo, &trans, &diag, &n, &k, &aa, &aoff, &lda, &xx, &xoff, &incx)
												} else if packed {
													Ztpmv(&uplo, &trans, &diag, &n, &aa, &aoff, &xx, &xoff, &incx)
												}
											} else if sname[3:5] == "SV" {
												if full {
													Ztrsv(&uplo, &trans, &diag, &n, &aa, &aoff, &lda, &xx, &xoff, &incx)
												} else if banded {
													Ztbsv(&uplo, &trans, &diag, &n, &k, &aa, &aoff, &lda, &xx, &xoff, &incx)
												} else if packed {
													Ztpsv(&uplo, &trans, &diag, &n, &aa, &aoff, &xx, &xoff, &incx)
												}
											}
											//
											//                       Check if error-exit was taken incorrectly.
											...
											if full {
												...
												if null {
													...
													isame[6] = lzeres("GE", ' ', &ione, &n, &xs, &xx, &xoff, func() *int { y := absint(incx); return &y }())
												}
												isame[7] = incxs == incx
											} else if banded {
												isame[4] = ks == k
												isame[5] = lze(&as, &aa, &aoff, &laa)
												isame[6] = ldas == lda
												if null {
													isame[7] = lze(&xs, &xx, &xoff, &lx)
												} else {
													isame[7] = lzeres("GE", ' ', &ione, &n, &xs, &xx, &xoff, func() *int { y := absint(incx); return &y }())
												}
												isame[8] = incxs == incx
											} else if packed {
												isame[4] = lze(&as, &aa, &aoff, &laa)
												if null {
													...
													isame[5] = lzeres("GE", ' ', &ione, &n, &xs, &xx, &xoff, func() *int { y := absint(incx); return &y }())
												}
												isame[6] = incxs == incx
											}
											//
											...
											if !null {
												if sname[3:5] == "MV" {
													...
													//                             Check the result.
													//
													zmvch(trans, &n, &n, &one, &a, &aoff, &nmax, &x, &xoff, &incx, &zero, &z, &izero, &incx, &xt, &g, &xx, &xoff, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
												} else if sname[3:5] == "SV" {
													//
													...
													zmvch(trans, &n, &n, &one, &a, &aoff, &nmax, &z, &izero, &incx, &zero, &x, &xoff, &incx, &xt, &g, &xx, &xoff, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
												}
												errmax = maxf64(errmax, err)
						...
						for _, n = range idim {
							...
							for im = 1; im <= 2; im++ {
								...
								for _, incx = range inc {
									...
									//
									transl = half
									zmakeL2("GE", ' ', ' ', &ione, &m, &x, &ione, &xx, &xoff, func() *int { y := absint(incx); return &y }(), &izero, func() *int { y := m - 1; return &y }(), &reset, &transl)
									if m > 1 {
										x[m/2-1+xoff] = zero
									...
									for _, incy = range inc {
										...
										for _, alpha = range alf {
											...
											//
											transl = zero
											zmakeL2(sname[1:3], ' ', ' ', &m, &n, &a, &nmax, &aa, &aoff, &lda, func() *int { y := m - 1; return &y }(), func() *int { y := n - 1; return &y }(), &reset, &transl)
											...
											//
											if conj {
												Zgerc(&m, &n, &alpha, &xx, &xoff, &incx, &yy, &yoff, &incy, &aa, &aoff, &lda)
											} else {
												Zgeru(&m, &n, &alpha, &xx, &xoff, &incx, &yy, &yoff, &incy, &aa, &aoff, &lda)
											}
											//
											//                    Check if error-exit was taken incorrectly.
											//
											...
											isame[1] = ns == n
											isame[2] = als == alpha
											isame[3] = lze(&xs, &xx, &xoff, &lx)
											isame[4] = incxs == incx
											isame[5] = lze(&ys, &yy, &yoff, &ly)
											isame[6] = incys == incy
											if null {
												isame[7] = lze(&as, &aa, &aoff, &laa)
											} else {
												isame[7] = lzeres("GE", ' ', &m, &n, &as, &aa, &aoff, &lda)
											}
											isame[8] = ldas == lda
											//
											...
											if !null {
												...
												for j = 1; j <= n; j++ {
													...
													zmvch('N', &m, &ione, &alpha, &z, &izero, &nmax, &w, &izero, &ione, &one, &a, func() *int { y := 0 + (j-1)*nmax + aoff; return &y }(), &ione, &yt, &g, &aa, func() *int { y := 1 + (j-1)*lda - 1 + aoff; return &y }(), &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													errmax = maxf64(errmax, err)
													//                          If got really bad answer, report and return.
													if fatal {
														goto label3130
						...
						for _, uplo = range ichu {
							...
							for _, incx = range inc {
								...
								for _, ralpha = range ralf {
									...
									//
									transl = zero
									zmakeL2(sname[1:3], uplo, ' ', &n, &n, &a, &nmax, &aa, &aoff, &lda, func() *int { y := n - 1; return &y }(), func() *int { y := n - 1; return &y }(), &reset, &transl)
									...
									//
									if full {
										Zher(&uplo, &n, &ralpha, &xx, &xoff, &incx, &aa, &aoff, &lda)
									} else if packed {
										Zhpr(&uplo, &n, &ralpha, &xx, &xoff, &incx, &aa, &aoff)
									}
									//
									//                 Check if error-exit was taken incorrectly.
									//
									...
									if null {
										...
										isame[5] = lzeres(sname[1:3], uplo, &n, &n, &as, &aa, &aoff, &lda)
									}
									if !packed {
										isame[6] = ldas == lda
									}
									...
									if !null {
										...
										for j = 1; j <= n; j++ {
											...
											zmvch('N', &lj, &ione, &alpha, &z, func() *int { y := jj - 1; return &y }(), &lj, &w, &izero, &ione, &one, &a, func() *int { y := jj - 1 + (j-1)*nmax + aoff; return &y }(), &ione, &yt, &g, &aa, func() *int { y := ja - 1 + aoff; return &y }(), &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
											if full {
												if upper {
													ja = ja + lda
												} else {
								...
								for _, incx = range inc {
									...
									for _, incy = range inc {
										...
										for _, alpha = range alf {
											...
											//
											if full {
												Zher2(&uplo, &n, &alpha, &xx, &xoff, &incx, &yy, &yoff, &incy, &aa, &aoff, &lda)
											} else if packed {
												Zhpr2(&uplo, &n, &alpha, &xx, &xoff, &incx, &yy, &yoff, &incy, &aa, &aoff)
											}
											//
											//                    Check if error-exit was taken incorrectly.
											//
											...
											if null {
												...
												isame[7] = lzeres(sname[1:3], uplo, &n, &n, &as, &aa, &aoff, &lda)
											}
											if !packed {
												isame[8] = ldas == lda
											}
											...
											if !null {
												...
												ja = 1
												for j = 1; j <= n; j++ {
													w[0] = alpha * conjc128(z[j-1+1*nmax])
													w[1] = conjc128(alpha) * conjc128(z[j-1+0*nmax])
													if upper {
														jj = 1
														lj = j
													} else {
													...
													zmvch('N', &lj, &itwo, &one, &z, func() *int { y := jj - 1 + 0*nmax; return &y }(), &nmax, &w, &izero, &ione, &one, &a, func() *int { y := jj - 1 + (j-1)*nmax + aoff; return &y }(), &ione, &yt, &g, &aa, func() *int { y := ja - 1 + aoff; return &y }(), &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													if full {
														if upper {
															ja = ja + lda
														} else {
...
func TestZblasLevel3(t *testing.T) {
	...
	var alpha, als, beta, bets, bls complex128
	var err, ralpha, rals, rbeta, rbets, rbls float64
	var i, j, n int
	var ok bool = true
	var reset bool = true
	var rzero float64 = 0.0
	...
	var ichu []byte = []byte{'U', 'L'}
	a := make([]complex128, nmax*2*nmax)
	aa := make([]complex128, nmax*nmax)
	ab := make([]complex128, nmax*2*nmax)
	as := make([]complex128, nmax*nmax)
	b := make([]complex128, nmax*2*nmax)
	bb := make([]complex128, nmax*nmax)
	bs := make([]complex128, nmax*nmax)
	c := make([]complex128, nmax*nmax)
	cc := make([]complex128, nmax*nmax)
	cs := make([]complex128, nmax*nmax)
	ct := make([]complex128, nmax)
	g := make([]float64, nmax)
	w := make([]complex128, 2*nmax)
	var snames []string = []string{"ZGEMM", "ZHEMM", "ZSYMM", "ZTRMM", "ZTRSM", "ZHERK", "ZSYRK", "ZHER2K", "ZSYR2K"}
	...
	for j = 1; j <= n; j++ {
		...
		a[j-1+(nmax)*nmax] = complex(float64(j), 0.0)
		a[0+(nmax+j-1)*nmax] = complex(float64(j), 0.0)
	...
	for j = 1; j <= n; j++ {
		cc[j-1] = complex(float64(j*((j+1)*j))/2-float64((j+1)*j*(j-1))/3, 0.0)
	}
	//     CC holds the exact result. On exit from SMMCH CT holds
	//     the result computed by SMMCH.
	transa = byte('N')
	transb = byte('N')
	zmmch(transa, transb, &n, func() *int { y := 1; return &y }(), &n, &one, &a, &izero, &nmax, &a, func() *int { y := nmax * nmax; return &y }(), &nmax, &zero, &c, &izero, &nmax, &ct, &g, &cc, &izero, &nmax, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
	same = lze(&cc, &ct, &izero, &n)
	if !same || err != rzero {
	...
	for j = 1; j <= n; j++ {
		a[j-1+(nmax)*nmax] = complex(float64(n-j+1), 0.0)
		a[0+(nmax+j-1)*nmax] = complex(float64(n-j+1), 0.0)
		ab[j-1+(nmax)*nmax] = a[j-1+(nmax)*nmax]
		ab[0+(nmax+j-1)*nmax] = a[0+(nmax+j-1)*nmax]
		b[j-1+(nmax)*nmax] = a[j-1+(nmax)*nmax]
		b[0+(nmax+j-1)*nmax] = a[0+(nmax+j-1)*nmax]
	...
	for j = 1; j <= n; j++ {
		cc[n-j+1-1] = complex(float64(j*((j+1)*j))/2-float64((j+1)*j*(j-1))/3, 0.0)
	}
	transa = byte('C')
	transb = byte('N')
	zmmch(transa, transb, &n, func() *int { y := 1; return &y }(), &n, &one, &ab, &izero, &nmax, &ab, func() *int { y := nmax * nmax; return &y }(), &nmax, &zero, &c, &izero, &nmax, &ct, &g, &cc, &izero, &nmax, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
	same = lze(&cc, &ct, &izero, &n)
	if !same || err != rzero {
		t.Errorf(" ERROR IN ZMMCH -  IN-LINE DOT PRODUCTS ARE BEING EVALUATED WRONGLY.\n ZMMCH WAS CALLED WITH TRANSA = %c AND TRANSB = %c\n AND RETURNED SAME =  %t AND ERR = %12.3f.\n THIS MAY BE DUE TO FAULTS IN THE ARITHMETIC OR THE COMPILER.\n ******* TESTS ABANDONED *******\n", transa, transb, same, err)
	}
	transb = byte('C')
	zmmch(transa, transb, &n, func() *int { y := 1; return &y }(), &n, &one, &ab, &izero, &nmax, &ab, func() *int { y := nmax * nmax; return &y }(), &nmax, &zero, &c, &izero, &nmax, &ct, &g, &cc, &izero, &nmax, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
	same = lze(&cc, &ct, &izero, &n)
	...
	for _, sname := range snames {
							...
							for _, n = range idim {
								...
								for _, k = range idim {
									...
									for _, transa = range icht {
										...
										//                 Generate the matrix A.
										//
										zmakeL3("GE", ' ', ' ', &ma, &na, &a, &aoff, &nmax, &aa, &aoff, &lda, &reset, &zero)
										...
										for _, transb = range icht {
											...
											//
											zmakeL3("GE", ' ', ' ', &mb, &nb, &b, &boff, &nmax, &bb, &boff, &ldb, &reset, &zero)
											...
											for _, alpha = range alf {
												...
												for _, beta = range bet {
													...
													//
													zmakeL3("GE", ' ', ' ', &m, &n, &c, &coff, &nmax, &cc, &coff, &ldc, &reset, &zero)
													...
													//                          Call the subroutine.
													//
													Zgemm(&transa, &transb, &m, &n, &k, &alpha, &aa, &aoff, &lda, &bb, &boff, &ldb, &beta, &cc, &coff, &ldc)
													//
													//                          Check if error-exit was taken incorrectly.
													//
													if !ok {
													...
													isame[6] = lze(&as, &aa, &aoff, &laa)
													isame[7] = ldas == lda
													isame[8] = lze(&bs, &bb, &boff, &lbb)
													isame[9] = ldbs == ldb
													isame[10] = bls == beta
													if null {
														isame[11] = lze(&cs, &cc, &coff, &lcc)
													} else {
														isame[11] = lzeres("GE", ' ', &m, &n, &cs, &cc, &coff, &ldc)
													}
													isame[12] = ldcs == ldc
													//
													//                          If data was incorrectly changed, report
													...
													if !null {
														...
														//                             Check the result.
														//
														zmmch(transa, transb, &m, &n, &k, &alpha, &a, &aoff, &nmax, &b, &boff, &nmax, &beta, &c, &coff, &nmax, &ct, &g, &cc, &coff, &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
														errmax = maxf64(errmax, err)
														//                             If got really bad answer, report and
														//                             return.
														if fatal {
					...
					for coff = 0; coff <= 30; coff += 10 {
						for _, m = range idim {
							...
							for _, n = range idim {
								...
								//           Generate the matrix B.
								//
								zmakeL3("GE", ' ', ' ', &m, &n, &b, &boff, &nmax, &bb, &boff, &ldb, &reset, &zero)
								...
								for _, side = range ichs {
									...
									for _, uplo = range ichu {
										...
										//
										zmakeL3(sname[1:3], uplo, ' ', &na, &na, &a, &aoff, &nmax, &aa, &aoff, &lda, &reset, &zero)
										...
										for _, alpha = range alf {
											...
											for _, beta = range bet {
												...
												//
												if conj {
													Zhemm(&side, &uplo, &m, &n, &alpha, &aa, &aoff, &lda, &bb, &boff, &ldb, &beta, &cc, &coff, &ldc)
												} else {
													Zsymm(&side, &uplo, &m, &n, &alpha, &aa, &aoff, &lda, &bb, &boff, &ldb, &beta, &cc, &coff, &ldc)
												}
												//
												//                       Check if error-exit was taken incorrectly.
												//
												...
												isame[5] = lze(&as, &aa, &aoff, &laa)
												isame[6] = ldas == lda
												isame[7] = lze(&bs, &bb, &boff, &lbb)
												isame[8] = ldbs == ldb
												isame[9] = bls == beta
												if null {
													isame[10] = lze(&cs, &cc, &coff, &lcc)
												} else {
													isame[10] = lzeres("GE", ' ', &m, &n, &cs, &cc, &coff, &ldc)
												}
												isame[11] = ldcs == ldc
												//
												//                       If data was incorrectly changed, report and
												...
												if !null {
													...
													//
													if left {
														zmmch('N', 'N', &m, &n, &m, &alpha, &a, &aoff, &nmax, &b, &boff, &nmax, &beta, &c, &coff, &nmax, &ct, &g, &cc, &coff, &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													} else {
														zmmch('N', 'N', &m, &n, &n, &alpha, &b, &boff, &nmax, &a, &aoff, &nmax, &beta, &c, &coff, &nmax, &ct, &g, &cc, &coff, &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													}
													errmax = maxf64(errmax, err)
													//                          If got really bad answer, report and
													//                          return.
									...
									for _, transa = range icht {
										...
										for _, diag = range ichd {
											...
											for _, alpha = range alf {
												...
												//                          Generate the matrix A.
												//
												zmakeL3("TR", uplo, diag, &na, &na, &a, &aoff, &nmax, &aa, &aoff, &lda, &reset, &zero)
												//
												//                          Generate the matrix B.
												//
												zmakeL3("GE", ' ', ' ', &m, &n, &b, &boff, &nmax, &bb, &boff, &ldb, &reset, &zero)
												...
												//
												if sname[3:5] == "MM" {
													Ztrmm(&side, &uplo, &transa, &diag, &m, &n, &alpha, &aa, &aoff, &lda, &bb, &boff, &ldb)
												} else if sname[3:5] == "SM" {
													Ztrsm(&side, &uplo, &transa, &diag, &m, &n, &alpha, &aa, &aoff, &lda, &bb, &boff, &ldb)
												}
												//
												//                          Check if error-exit was taken incorrectly.
												//
												...
												isame[8] = ldas == lda
												if null {
													isame[9] = lze(&bs, &bb, &boff, &lbb)
												} else {
													isame[9] = lzeres("GE", ' ', &m, &n, &bs, &bb, &boff, &ldb)
												}
												isame[10] = ldbs == ldb
												//
												//                          If data was incorrectly changed, report and
												...
												if !null {
													if string(sname[3:5]) == "MM" {
														...
														//
														if left {
															zmmch(transa, 'N', &m, &n, &m, &alpha, &a, &aoff, &nmax, &b, &boff, &nmax, &zero, &c, &izero, &nmax, &ct, &g, &bb, &boff, &ldb, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
														} else {
															zmmch('N', transa, &m, &n, &n, &alpha, &b, &boff, &nmax, &a, &aoff, &nmax, &zero, &c, &izero, &nmax, &ct, &g, &bb, &boff, &ldb, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
														}
													} else if sname[3:5] == "SM" {
														//
														...
														if left {
															zmmch(transa, 'N', &m, &n, &m, &one, &a, &aoff, &nmax, &c, &izero, &nmax, &zero, &b, &boff, &nmax, &ct, &g, &bb, &boff, &ldb, &eps, &err, &fatal, func() *bool { y := false; return &y }(), t)
														} else {
															zmmch('N', transa, &m, &n, &n, &one, &c, &izero, &nmax, &a, &aoff, &nmax, &zero, &b, &boff, &nmax, &ct, &g, &bb, &boff, &ldb, &eps, &err, &fatal, func() *bool { y := false; return &y }(), t)
														}
													}
													errmax = maxf64(errmax, err)
													//                             If got really bad answer, report and
		...
		label3150:
			...
			var nargs int = 10
			var nc int = 0
			var conj bool = string(sname[1:3]) == "HE"
								...
								for _, uplo = range ichu {
									...
									for _, alpha = range alf {
										...
										for _, beta = range bet {
											...
											//                       Generate the matrix C.
											//
											zmakeL3(sname[1:3], uplo, ' ', &n, &n, &c, &coff, &nmax, &cc, &coff, &ldc, &reset, &zero)
											...
											//
											if conj {
												Zherk(&uplo, &trans, &n, &k, &ralpha, &aa, &aoff, &lda, &rbeta, &cc, &coff, &ldc)
											} else {
												Zsyrk(&uplo, &trans, &n, &k, &alpha, &aa, &aoff, &lda, &beta, &cc, &coff, &ldc)
											}
											//
											//                       Check if error-exit was taken incorrectly.
											//
											...
											if null {
												isame[8] = lze(&cs, &cc, &coff, &lcc)
											} else {
												isame[8] = lzeres(sname[1:3], uplo, &n, &n, &cs, &cc, &coff, &ldc)
											}
											isame[9] = ldcs == ldc
											//
											//                       If data was incorrectly changed, report and
											...
											if !null {
												...
												for j = 1; j <= n; j++ {
													...
													if tran {
														zmmch(transt, 'N', &lj, func() *int { y := 1; return &y }(), &k, &alpha, &a, func() *int { y := 0 + (jj-1)*nmax + aoff; return &y }(), &nmax, &a, func() *int { y := 0 + (j-1)*nmax + aoff; return &y }(), &nmax, &beta, &c, func() *int { y := jj - 1 + (j-1)*nmax + coff; return &y }(), &nmax, &ct, &g, &cc, func() *int { y := jc - 1 + coff; return &y }(), &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													} else {
														zmmch('N', transt, &lj, func() *int { y := 1; return &y }(), &k, &alpha, &a, func() *int { y := jj - 1 + (0)*nmax + aoff; return &y }(), &nmax, &a, func() *int { y := j - 1 + (0)*nmax + aoff; return &y }(), &nmax, &beta, &c, func() *int { y := jj - 1 + (j-1)*nmax + coff; return &y }(), &nmax, &ct, &g, &cc, func() *int { y := jc - 1 + coff; return &y }(), &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
													}
													if upper {
														jc = jc + ldc
													} else {
				...
				for _, n = range idim {
					...
					for _, k = range idim {
						...
						for _, trans = range icht {
							...
							//
							if tran {
								zmakeL3("GE", ' ', ' ', &ma, &na, &ab, &izero, func() *int { y := 2 * nmax; return &y }(), &aa, &izero, &lda, &reset, &zero)
							} else {
								zmakeL3("GE", ' ', ' ', &ma, &na, &ab, &izero, &nmax, &aa, &izero, &lda, &reset, &zero)
							}
							//
							//              Generate the matrix B.
							//
							...
							lbb = laa
							if tran {
								zmakeL3("GE", ' ', ' ', &ma, &na, &ab, &k, func() *int { y := 2 * nmax; return &y }(), &bb, &izero, &ldb, &reset, &zero)
							} else {
								zmakeL3("GE", ' ', ' ', &ma, &na, &ab, func() *int { y := k * nmax; return &y }(), &nmax, &bb, &izero, &ldb, &reset, &zero)
							}
							...
							for _, uplo = range ichu {
								...
								for _, alpha = range alf {
									...
									for _, beta = range bet {
										...
										//
										if conj {
											Zher2k(&uplo, &trans, &n, &k, &alpha, &aa, &izero, &lda, &bb, &izero, &ldb, &rbeta, &cc, &coff, &ldc)
										} else {
											Zsyr2k(&uplo, &trans, &n, &k, &alpha, &aa, &izero, &lda, &bb, &izero, &ldb, &beta, &cc, &coff, &ldc)
										}
										//
										//                       Check if error-exit was taken incorrectly.
										//
										...
										isame[3] = ks == k
										isame[4] = als == alpha
										isame[5] = lze(&as, &aa, &izero, &laa)
										isame[6] = ldas == lda
										isame[7] = lze(&bs, &bb, &izero, &lbb)
										isame[8] = ldbs == ldb
										if conj {
											isame[9] = rbets == rbeta
										} else {
										...
										if null {
											...
											isame[10] = lzeres("HE", uplo, &n, &n, &cs, &cc, &coff, &ldc)
										}
										isame[11] = ldcs == ldc
										//
										//                       If data was incorrectly changed, report and
											...
											for j = 1; j <= n; j++ {
												...
												if tran {
													for i = 1; i <= k; i++ {
														...
														if conj {
															w[k+i-1] = conjc128(alpha) * ab[(j-1)*2*nmax+i-1]
														} else {
															w[k+i-1] = alpha * ab[(j-1)*2*nmax+i-1]
														}
													}
													zmmch(transt, 'N', &lj, func() *int { y := 1; return &y }(), func() *int { y := 2 * k; return &y }(), &one, &ab, func() *int { y := jjab - 1; return &y }(), func() *int { y := 2 * nmax; return &y }(), &w, &izero, func() *int { y := 2 * nmax; return &y }(), &beta, &c, func() *int { y := jj - 1 + (j-1)*nmax + coff; return &y }(), &nmax, &ct, &g, &cc, func() *int { y := jc - 1 + coff; return &y }(), &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
												} else {
													for i = 1; i <= k; i++ {
														if conj {
															w[i-1] = alpha * conjc128(ab[(k+i-1)*nmax+j-1])
															w[k+i-1] = conjc128(alpha * ab[(i-1)*nmax+j-1])
														} else {
															w[i-1] = alpha * ab[(k+i-1)*nmax+j-1]
															w[k+i-1] = alpha * ab[(i-1)*nmax+j-1]
														}
													}
													zmmch('N', 'N', &lj, func() *int { y := 1; return &y }(), func() *int { y := 2 * k; return &y }(), &one, &ab, func() *int { y := jj - 1; return &y }(), &nmax, &w, &izero, func() *int { y := 2 * nmax; return &y }(), &beta, &c, func() *int { y := jj - 1 + (j-1)*nmax + coff; return &y }(), &nmax, &ct, &g, &cc, func() *int { y := jc - 1 + coff; return &y }(), &ldc, &eps, &err, &fatal, func() *bool { y := true; return &y }(), t)
												}
												if upper {
													jc = jc + ldc
												} else {
...
func zcompare(len int, ccomp *[]complex128, ccompoff int, ctrue *[]complex128, ctrueoff int, csize *[]complex128, csizeoff int, sfac float64, t *testing.T, fname string) {
	var i int
	...
	dcompare(2*len, &scomp, 0, &strue, 0, &ssize, 0, sfac, t, fname)
}

func zbeg(reset *bool) (cbegReturn complex128) {
...
label10:
	...
	*j = (*j) - 1000*((*j)/1000)
	...
	cbegReturn = complex((float64(*i)-500)/1001.0, (float64(*j)-500)/1001.0)
	return
}

func lze(ri *[]complex128, rj *[]complex128, roff, lr *int) bool {
	var i int
...
func lzeres(_type string, uplo byte, m *int, n *int, aa *[]complex128, as *[]complex128, aoff, lda *int) bool {
	var upper bool
...
func zmakeL2(_type string, uplo byte, diag byte, m *int, n *int, a *[]complex128, nmax *int, aa *[]complex128, aoff, lda *int, kl *int, ku *int, reset *bool, transl *complex128) {
	var i, i1, i2, i3, ibeg, iend, ioff, j, jj, kk int
	var zero complex128 = complex(0.0, 0.0)
	var one complex128 = complex(1.0, 0.0)
	var rogue complex128 = complex(-1e10, 1e10)
	var rzero float64 = 0.0
	var rrogue float64 = -1e10
		...
		for i = 1; i <= (*m); i++ {
			if gen || (upper && i <= j) || (lower && i >= j) {
				if (i <= j && j-i <= (*ku)) || (i >= j && i-j <= (*kl)) {
					(*a)[i-1+(j-1)*(*nmax)+(*aoff)] = zbeg(reset) + (*transl)
				} else {
					(*a)[i-1+(j-1)*(*nmax)+(*aoff)] = zero
				}
				if i != j {
					if sym {
						(*a)[j-1+(i-1)*(*nmax)+(*aoff)] = conjc128((*a)[i-1+(j-1)*(*nmax)+(*aoff)])
					} else if tri {
...
func zmakeL3(_type string, uplo byte, diag byte, m *int, n *int, a *[]complex128, aoff, nmax *int, aa *[]complex128, aaoff, lda *int, reset *bool, transl *complex128) {
	var i, ibeg, iend, j, jj int
	var zero complex128 = (0.0 + 0.0*1i)
	var one complex128 = (1.0 + 0.0*1i)
	var rogue complex128 = (-1e10 + 1e10*1i)
	var rzero float64 = 0.0
	var rrogue float64 = -1e10
...
func zmmch(transa byte, transb byte, m *int, n *int, kk *int, alpha *complex128, a *[]complex128, aoff, lda *int, b *[]complex128, boff, ldb *int, beta *complex128, c *[]complex128, coff, ldc *int, ct *[]complex128, g *[]float64, cc *[]complex128, ccoff, ldcc *int, eps *float64, err *float64, fatal *bool, mv *bool, t *testing.T) {
	var erri float64
	var i, j, k int
	var zero complex128 = (0.0 + 0.0*1i)
	var rzero float64 = 0.0
	...
	for j = 1; j <= (*n); j++ {
		...
		if !trana && !tranb {
			for k = 1; k <= (*kk); k++ {
				for i = 1; i <= (*m); i++ {
					...
					(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[i-1+(k-1)*(*lda)+(*aoff)])*abssumc128((*b)[k-1+(j-1)*(*ldb)+(*boff)])
				}
			}
		} else if trana && !tranb {
			if ctrana {
				for k = 1; k <= (*kk); k++ {
					for i = 1; i <= (*m); i++ {
						(*ct)[i-1] = (*ct)[i-1] + conjc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*(*b)[k-1+(j-1)*(*ldb)+(*boff)]
						(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*abssumc128((*b)[k-1+(j-1)*(*ldb)+(*boff)])
					}
				}
			} else {
				for k = 1; k <= (*kk); k++ {
			...
			if ctranb {
				for k = 1; k <= (*kk); k++ {
					for i = 1; i <= (*m); i++ {
						(*ct)[i-1] = (*ct)[i-1] + (*a)[i-1+(k-1)*(*lda)+(*aoff)]*conjc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
						(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[i-1+(k-1)*(*lda)+(*aoff)])*abssumc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
					}
				}
			} else {
				for k = 1; k <= (*kk); k++ {
			...
			if ctrana {
				if ctranb {
					for k = 1; k <= (*kk); k++ {
						for i = 1; i <= (*m); i++ {
							(*ct)[i-1] = (*ct)[i-1] + conjc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*conjc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
							(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*abssumc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
						}
					}
				} else {
					for k = 1; k <= (*kk); k++ {
						for i = 1; i <= (*m); i++ {
							(*ct)[i-1] = (*ct)[i-1] + conjc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*(*b)[j-1+(k-1)*(*ldb)+(*boff)]
							(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*abssumc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
						}
					}
				}
				...
				if ctranb {
					for k = 1; k <= (*kk); k++ {
						for i = 1; i <= (*m); i++ {
							(*ct)[i-1] = (*ct)[i-1] + (*a)[k-1+(i-1)*(*lda)+(*aoff)]*conjc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
							(*g)[i-1] = (*g)[i-1] + abssumc128((*a)[k-1+(i-1)*(*lda)+(*aoff)])*abssumc128((*b)[j-1+(k-1)*(*ldb)+(*boff)])
						}
					}
				} else {
		...
		for i = 1; i <= (*m); i++ {
			...
			(*g)[i-1] = abssumc128(*alpha)*(*g)[i-1] + abssumc128(*beta)*abssumc128((*c)[i-1+(j-1)*(*ldc)+(*coff)])
		}
		//
		...
		(*err) = rzero
		for i = 1; i <= (*m); i++ {
			erri = abssumc128((*ct)[i-1]-(*cc)[i-1+(j-1)*(*ldcc)+(*ccoff)]) / (*eps)
			if (*g)[i-1] != rzero {
				erri = erri / (*g)[i-1]
			}
			(*err) = maxf64(*err, erri)
...
func zmvch(trans byte, m *int, n *int, alpha *complex128, a *[]complex128, aoff, nmax *int, x *[]complex128, xoff, incx *int, beta *complex128, y *[]complex128, yoff, incy *int, yt *[]complex128, g *[]float64, yy *[]complex128, yyoff *int, eps *float64, err *float64, fatal *bool, mv *bool, t *testing.T) {
	var ctran, tran bool
	var erri float64
	...
	for i = 1; i <= ml; i++ {
		...
		if tran {
			for j = 1; j <= nl; j++ {
				...
				(*g)[iy-1] = (*g)[iy-1] + abssumc128((*a)[j-1+(i-1)*(*nmax)+(*aoff)])*abssumc128((*x)[jx-1+(*xoff)])
				jx = jx + incxl
			}
		} else if ctran {
			for j = 1; j <= nl; j++ {
				(*yt)[iy-1] = (*yt)[iy-1] + conjc128((*a)[j-1+(i-1)*(*nmax)+(*aoff)])*(*x)[jx-1+(*xoff)]
				(*g)[iy-1] = (*g)[iy-1] + abssumc128((*a)[j-1+(i-1)*(*nmax)+(*aoff)])*abssumc128((*x)[jx-1+(*xoff)])
				jx = jx + incxl
			}
		} else {
			for j = 1; j <= nl; j++ {
				...
				(*g)[iy-1] = (*g)[iy-1] + abssumc128((*a)[i-1+(j-1)*(*nmax)+(*aoff)])*abssumc128((*x)[jx-1+(*xoff)])
				jx = jx + incxl
			}
		}
		(*yt)[iy-1] = (*alpha)*(*yt)[iy-1] + (*beta)*(*y)[iy-1+(*yoff)]
		(*g)[iy-1] = (absf64(real(*alpha))+absf64(imag(*alpha)))*(*g)[iy-1] + (absf64(real(*beta))+absf64(imag(*beta)))*abssumc128((*y)[iy-1+(*yoff)])
		iy = iy + incyl
	}
	//
	//     Compute the error ratio for this result.
	...
	(*err) = rzero
	for i = 1; i <= ml; i++ {
		erri = absc128((*yt)[i-1]-(*yy)[1+(i-1)*absint(*incy)-1+(*yyoff)]) / (*eps)
		if (*g)[i-1] != rzero {
			erri = erri / (*g)[i-1]
		}
		(*err) = maxf64(*err, erri)
