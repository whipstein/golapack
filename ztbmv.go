...
//
//  =====================================================================
func Ztbmv(uplo *byte, trans *byte, diag *byte, n *int, k *int, a *[]complex128, aoff, lda *int, x *[]complex128, xoff, incx *int) {
			...
			if (*incx) == 1 {
				for j = (*n); j >= 1; j-- {
					...
					if noconj {
						...
						if nounit {
							temp = temp * conjc128((*a)[kplus1-1+(j-1)*(*lda)+(*aoff)])
						}
						for i = j - 1; i >= maxint(1, j-(*k)); i-- {
							temp = temp + conjc128((*a)[l+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[i-1+(*xoff)]
						}
					}
					(*x)[j-1+(*xoff)] = temp
				}
				...
				for j = (*n); j >= 1; j-- {
					...
					if noconj {
						...
						for i = j - 1; i >= maxint(1, j-(*k)); i-- {
							temp = temp + conjc128((*a)[l+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[ix-1+(*xoff)]
							ix = ix - (*incx)
						}
					}
					(*x)[jx-1+(*xoff)] = temp
			...
			if (*incx) == 1 {
				for j = 1; j <= (*n); j++ {
					...
					if noconj {
						...
						if nounit {
							temp = temp * conjc128((*a)[0+(j-1)*(*lda)+(*aoff)])
						}
						for i = j + 1; i <= minint(*n, j+(*k)); i++ {
							temp = temp + conjc128((*a)[l+i-1+(j-1)*(*lda)+(*aoff)])*(*x)[i-1+(*xoff)]
						}
