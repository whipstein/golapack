...
// Ddrvsyaa2Stage tests the driver routine Dsysvaa2Stage.
//
//  =========== DOCUMENTATION ===========
//
...
//
//  =====================================================================
func Ddrvsyaa2Stage(dotype *[]bool, nn *int, nval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a *[]float64, afac *[]float64, ainv *[]float64, b *[]float64, x *[]float64, xact *[]float64, work *[]float64, rwork *[]float64, iwork *[]int, nout *int) {
	one := new(float64)
	zero := new(float64)
	ntypes := new(int)
	ntests := new(int)
			...
			for (*iuplo) = 1; (*iuplo) <= 2; (*iuplo)++ {
				...
				for (*ifact) = 1; (*ifact) <= (*nfact); (*ifact)++ {
					...
					if (*ifact) == 2 {
						...
						//
						(*srnamt) = *func() *[]byte {y :=[]byte("Dsysvaa2Stage "); return &y }()
						(*lwork) = (Min((*n)*(*nb), 3*(*(nmax))*(*(nmax))))
						Dsysvaa2Stage(uplo, n, (nrhs), (afac), lda, (ainv), (3*(*nb)+1)*(*n), (iwork), &((*(iwork))[1+(*n)-1]), (x), lda, (work), lwork, info)
						//
						...
						Dpot02(uplo, n, (nrhs), (a), lda, (x), lda, (work), lda, (rwork), &((*result)[0]))
						//
