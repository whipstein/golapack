...
//
//  =====================================================================
func dlavsy(uplo *byte, trans *byte, diag *byte, n *int, nrhs *int, a *[][]float64, lda *int, ipiv *[]int, b *[][]float64, ldb *int, info *int) {
	one := new(float64)
	nounit := new(bool)
	j := new(int)
	k := new(int)
	...
	if (*(info)) != 0 {
		Xerbla(func() *[]byte {y := []byte("dlavsy "); return &y }(), -(*(info)))
		return
	}
	//
