...
//
//  =====================================================================
func Dchkpo(dotype *[]bool, nn *int, nval *[]int, nnb *int, nbval *[]int, nns *int, nsval *[]int, thresh *float64, tsterr *bool, nmax *int, a *[]float64, afac *[]float64, ainv *[]float64, b *[]float64, x *[]float64, xact *[]float64, work *[]float64, rwork *[]float64, iwork *[]int, nout *int) {
	zero := new(float64)
	ntypes := new(int)
	ntests := new(int)
	zerot := new(bool)
	...
	(*path)[1] = *func() *[]byte {y :=[]byte("PO"); return &y }()
	(*nrun) = 0
	(*nfail) = 0
	(*nerrs) = 0
	for (*i) = 1; (*i) <= 4; (*i)++ {
		...
		for (*imat) = 1; (*imat) <= (*nimat); (*imat)++ {
			...
			for (*iuplo) = 1; (*iuplo) <= 2; (*iuplo)++ {
				...
				for (*inb) = 1; (*inb) <= (*(nnb)); (*inb)++ {
					...
					(*srnamt) = *func() *[]byte {y :=[]byte("DPOTRF"); return &y }()
					Dpotrf(uplo, n, (afac), lda, info)
					//
					//                 Check error code from DPOTRF.
					//
					if (*info) != (*izero) {
						Alaerh(path, func() *[]byte {y :=[]byte("DPOTRF"); return &y }(), info, izero, uplo, n, n, -1, -1, nb, imat, nfail, nerrs, (nout))
						goto Label90
					}
					//
					...
					Dpot01(uplo, n, (a), lda, (ainv), lda, (rwork), &((*result)[0]))
					//
					//+    TEST 2
					//                 Form the inverse and compute the residual.
					//
					Dlacpy(uplo, n, n, (afac), lda, (ainv), lda)
					(*srnamt) = *func() *[]byte {y :=[]byte("DPOtri"); return &y }()
					Dpotri(uplo, n, (ainv), lda, info)
					//
					//                 Check error code from DPOtri.
					//
					if (*info) != 0 {
						Alaerh(path, func() *[]byte {y :=[]byte("DPOtri"); return &y }(), info, func() *int {y := 0; return &y }(), uplo, n, n, -1, -1, -1, imat, nfail, nerrs, (nout))
					}
					//
					...
					for (*irhs) = 1; (*irhs) <= (*(nns)); (*irhs)++ {
						...
						//
						(*srnamt) = *func() *[]byte {y :=[]byte("DPOTRS"); return &y }()
						Dpotrs(uplo, n, nrhs, (afac), lda, (x), lda, info)
						//
						//                 Check error code from DPOTRS.
						//
						if (*info) != 0 {
							Alaerh(path, func() *[]byte {y :=[]byte("DPOTRS"); return &y }(), info, func() *int {y := 0; return &y }(), uplo, n, n, -1, -1, nrhs, imat, nfail, nerrs, (nout))
						}
						//
						...
						//
						(*srnamt) = *func() *[]byte {y :=[]byte("DPORFS"); return &y }()
						Dporfs(uplo, n, nrhs, (a), lda, (afac), lda, (b), lda, (x), lda, (rwork), &((*(rwork))[(*nrhs)+0]), (work), (iwork), info)
						//
						//                 Check error code from DPORFS.
						//
						if (*info) != 0 {
							Alaerh(path, func() *[]byte {y :=[]byte("DPORFS"); return &y }(), info, func() *int {y := 0; return &y }(), uplo, n, n, -1, -1, nrhs, imat, nfail, nerrs, (nout))
						}
						//
						Dget04(n, nrhs, (x), lda, (xact), lda, rcondc, &((*result)[4]))
						Dpot05(uplo, n, nrhs, (a), lda, (b), lda, (x), lda, (xact), lda, (rwork), &((*(rwork))[(*nrhs)+0]), &((*result)[5]))
						//
						//                    Print information about the tests that did not pass
						//                    the threshold.
						//
					...
					(*srnamt) = *func() *[]byte {y :=[]byte("DPOCON"); return &y }()
					Dpocon(uplo, n, (afac), lda, anorm, rcond, (work), (iwork), info)
					//
					//                 Check error code from DPOCON.
					//
					if (*info) != 0 {
						Alaerh(path, func() *[]byte {y :=[]byte("DPOCON"); return &y }(), info, func() *int {y := 0; return &y }(), uplo, n, n, -1, -1, -1, imat, nfail, nerrs, (nout))
					}
					//
