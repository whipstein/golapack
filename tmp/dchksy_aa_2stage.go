...
//  ===========
//
//       SUBROUTinE DchksyAa2stage( dotype, nn, nval, nnb, nbval,
...
//
//  =====================================================================
func DchksyAa2stage(dotype *[]bool, nn *int, nval *[]int, nnb *int, nbval *[]int, nns *int, nsval *[]int, thresh *float64, tsterr *bool, nmax *int, a *[]float64, afac *[]float64, ainv *[]float64, b *[]float64, x *[]float64, xact *[]float64, work *[]float64, rwork *[]float64, iwork *[]int, nout *int) {
	zero := new(float64)
	one := new(float64)
	ntypes := new(int)
	ntests := new(int)
	...
	(*path)[1] = *func() *[]byte {y :=[]byte("S2"); return &y }()
	//
		...
		for (*imat) = 1; (*imat) <= (*nimat); (*imat)++ {
			...
			for (*iuplo) = 1; (*iuplo) <= 2; (*iuplo)++ {
				...
				for (*inb) = 1; (*inb) <= (*(nnb)); (*inb)++ {
					...
					//
					(*srnamt) = *func() *[]byte {y :=[]byte("Dsytrf_AA_2STAGE"); return &y }()
					(*lwork) = (Min((*n)*(*nb), 3*(*(nmax))*(*(nmax))))
					DsytrfAa2stage(uplo, n, (afac), lda, (ainv), (3*(*nb)+1)*(*n), (iwork), &((*(iwork))[1+(*n)-1]), (work), lwork, info)
					//
					...
					//
					if (*info) != (*k) {
						Alaerh(path, func() *[]byte {y :=[]byte("Dsytrf_AA_2STAGE"); return &y }(), info, k, uplo, n, n, -1, -1, nb, imat, nfail, nerrs, (nout))
					}
					//
					//+    TEST 1
					//                 Re_construct matrix from factors and compute residual.
					//
					//c                  CALL Dsyt01Aa( uplo, n, a, lda, afac, lda, iwork,
					//c     $                            ainv, lda, rwork, result1)
					//c                  NT = 1
					(*nt) = 0
					//
					//
					...
					for (*irhs) = 1; (*irhs) <= (*(nns)); (*irhs)++ {
						...
						//
						(*srnamt) = *func() *[]byte {y :=[]byte("Dsytrs_AA_2STAGE"); return &y }()
						(*lwork) = (MAX(1, 3*(*n)-2))
						DsytrsAa2stage(uplo, n, nrhs, (afac), lda, (ainv), (3*(*nb)+1)*(*n), (iwork), &((*(iwork))[1+(*n)-1]), (x), lda, info)
						//
						//                    Check error code from Dsytrs and handle error.
						//
						if (*info) != 0 {
							if (*izero) == 0 {
								Alaerh(path, func() *[]byte {y :=[]byte("Dsytrs_AA_2STAGE"); return &y }(), info, func() *int {y := 0; return &y }(), uplo, n, n, -1, -1, nrhs, imat, nfail, nerrs, (nout))
							}
						} else {
