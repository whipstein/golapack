...
//
//  =====================================================================
func Dchklq(dotype *[]bool, nm *int, mval *[]int, nn *int, nval *[]int, nnb *int, nbval *[]int, nxval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a *[]float64, af *[]float64, aq *[]float64, Al *[]float64, ac *[]float64, b *[]float64, x *[]float64, xact *[]float64, tau *[]float64, work *[]float64, rwork *[]float64, nout *int) {
	ntests := new(int)
	ntypes := new(int)
	zero := new(float64)
	dist := new(byte)
	...
	(*path)[1] = *func() *[]byte {y :=[]byte("LQ"); return &y }()
	(*nrun) = 0
	(*nfail) = 0
	(*nerrs) = 0
	for (*i) = 1; (*i) <= 4; (*i)++ {
				...
				for (*ik) = 1; (*ik) <= (*nk); (*ik)++ {
					...
					for (*inb) = 1; (*inb) <= (*(nnb)); (*inb)++ {
						...
						if (*ik) == 1 {
							...
							//                       Test Dgelqf
							//
							Dlqt01(m, n, (a), (af), (aq), (AL), lda, (tau), (work), lwork, (rwork), &((*result)[0]))
						} else if (*m) <= (*n) {
							//
							//                       Test Dorglq, using factorization
							//                       returned by Dlqt01
							//
							Dlqt02(m, n, k, (a), (af), (aq), (AL), lda, (tau), (work), lwork, (rwork), &((*result)[0]))
						} else {
							(*result)[0] = (*zero)
							(*result)[1] = (*zero)
						}
						if (*m) >= (*k) {
							...
							//                       by Dlqt01
							//
							Dlqt03(m, n, k, (af), (ac), (AL), (aq), lda, (tau), (work), lwork, (rwork), &((*result)[2]))
							(*nt) = (*nt) + 4
							//
							...
							if (*k) == (*m) && (*inb) == 1 {
								...
								Dgelqs(m, n, (nrhs), (af), lda, (tau), (x), lda, (work), lwork, info)
								//
								//                          Check error code from Dgelqs.
								//
								if (*info) != 0 {
									Alaerh(path, func() *[]byte {y :=[]byte("Dgelqs"); return &y }(), info, func() *int {y := 0; return &y }(), func() *byte {y := byte(' '); return &y }(), m, n, (nrhs), -1, nb, imat, nfail, nerrs, (nout))
								}
								//
