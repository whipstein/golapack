...
//
//  =====================================================================
func dlatmt(m *int, n *int, dist *byte, iseed *[]int, sym *byte, d *[]float64, mode *int, cond *float64, dmax *float64, rank *int, kl *int, ku *int, pack *byte, a *[][]float64, lda *int, work *[]float64, info *int) {
	zero := new(float64)
	one := new(float64)
	twopi := new(float64)
	alpha := new(float64)
	...
	//
	if (*(info)) != 0 {
		Xerbla(func() *[]byte {y := []byte("DLAtmT"); return &y }(), -(*(info)))
		return
	}
	//
	...
	if (*(mode)) != 0 && ABS((*(mode))) != 6 {
		...
		//
		Dscal((rank), alpha, (d), func() *int {y := 1; return &y }())
		//
	}
	//
