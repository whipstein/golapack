...
//
//  =====================================================================
func Dpbt02(uplo *byte, n *int, kd *int, nrhs *int, a *[][]float64, lda *int, x *[][]float64, ldx *int, b *[][]float64, ldb *int, rwork *[]float64, resid *float64) {
	zero := new(float64)
	one := new(float64)
	j := new(int)
	anorm := new(float64)
	...
	//
	for (*j) = 1; (*j) <= (*(nrhs)); (*j)++ {
		Dsbmv((uplo), (n), (kd), -(*one), (a), (lda), &((*(x))[0][(*j)-(1)]), func() *int {y := 1; return &y }(), one, &((*(b))[0][(*j)-(1)]), func() *int {y := 1; return &y }())
		//Label10:
	}
	//
