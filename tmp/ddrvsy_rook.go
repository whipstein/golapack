...
// DdrvsyRook tests the driver routines DsysvRook.
//
//  =========== DOCUMENTATION ===========
//
...
//  ===========
//
//     SUBROUTinE DdrvsyRook( dotype, nn, nval, nrhs, thresh, tsterr,
...
//
//  =====================================================================
func DdrvsyRook(dotype *[]bool, nn *int, nval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a *[]float64, afac *[]float64, ainv *[]float64, b *[]float64, x *[]float64, xact *[]float64, work *[]float64, rwork *[]float64, iwork *[]int, nout *int) {
	one := new(float64)
	zero := new(float64)
	ntypes := new(int)
	ntests := new(int)
			...
			for (*iuplo) = 1; (*iuplo) <= 2; (*iuplo)++ {
				...
				for (*ifact) = 1; (*ifact) <= (*nfact); (*ifact)++ {
					...
					if *zerot {
						...
						DsytrfRook(uplo, n, (afac), lda, (iwork), (work), lwork, info)
						//
					...
					if (*ifact) == 2 {
						...
						//                    DsysvRook.
						//
						(*srnamt) = *func() *[]byte {y :=[]byte("DsysvRook"); return &y }()
						DsysvRook(uplo, n, (nrhs), (afac), lda, (iwork), (x), lda, (work), lwork, info)
						//
						...
						//
						//                    Check error code from DsysvRook and handle error.
						//
						if (*info) != (*k) {
							Alaerh(path, func() *[]byte {y :=[]byte("DsysvRook"); return &y }(), info, k, uplo, n, n, -1, -1, (nrhs), imat, nfail, nerrs, (nout))
							goto Label120
						} else if (*info) != 0 {
							goto Label120
						}
						...
						for (*k) = 1; (*k) <= (*nt); (*k)++ {
							if (*result)[(*k)-1] >= (*(thresh)) {
								...
								WRITE((*(nout)), *func() *[]byte {y :=[]byte(" %s, uplo='%c', N =%5d, type %2d, test %2d, ratio =%12.5f\n"); return &y }(), *func() *[]byte {y :=[]byte("DsysvRook"); return &y }(), (*uplo), (*n), (*imat), (*k), (*result)[(*k)-1])
								(*nfail) = (*nfail) + 1
							}
							//Label110:
						}
