...
//
//  =====================================================================
func Dptt05(n *int, nrhs *int, d *[]float64, e *[]float64, b *[][]float64, ldb *int, x *[][]float64, ldx *int, xact *[][]float64, ldxact *int, ferr *[]float64, berr *[]float64, reslts *[]float64) {
	zero := new(float64)
	one := new(float64)
	i := new(int)
	imax := new(int)
	...
	for (*k) = 1; (*k) <= (*(nrhs)); (*k)++ {
		if (*(n)) == 1 {
			...
			(*axbi) = ABS(((*(b))[0][(*k)-(1)])) + ABS((*(d))[0]*(*(x))[0][(*k)-(1)]) + ABS((*(e))[0]*(*(x))[1][(*k)-(1)])
			for (*i) = 2; (*i) <= (*(n))-1; (*i)++ {
				(*tmp) = ABS(((*(b))[(*i)-(1)][(*k)-(1)])) + ABS((*(e))[(*i)-0]*(*(x))[(*i)-0][(*k)-(1)]) + ABS((*(d))[(*i)-(1)]*(*(x))[(*i)-(1)][(*k)-(1)]) + ABS((*(e))[(*i)-(1)]*(*(x))[(*i)+0][(*k)-(1)])
				(*axbi) = (Min((*axbi), (*tmp)))
				//Label40:
			}
			(*tmp) = ABS(((*(b))[(*(n))-(1)][(*k)-(1)])) + ABS((*(e))[(*(n))-0]*(*(x))[(*(n))-0][(*k)-(1)]) + ABS((*(d))[(*(n))-(1)]*(*(x))[(*(n))-(1)][(*k)-(1)])
			(*axbi) = (Min((*axbi), (*tmp)))
		}
