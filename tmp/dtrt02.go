...
//
//   =====================================================================
func Dtrt02(uplo *byte, trans *byte, diag *byte, n *int, nrhs *int, a *[][]float64, lda *int, x *[][]float64, ldx *int, b *[][]float64, ldb *int, work *[]float64, resid *float64) {
	zero := new(float64)
	one := new(float64)
	j := new(int)
	anorm := new(float64)
	...
	if blas.Lsame((trans), func() *byte {y := byte('N'); return &y }()) {
		...
		(*anorm) = (*Dlantr(func() *byte {y := byte('I'); return &y }(), (uplo), (diag), (n), (n), (a), (lda), (work)))
	}
	//
	...
	for (*j) = 1; (*j) <= (*(nrhs)); (*j)++ {
		...
		Dtrmv((uplo), (trans), (diag), (n), (a), (lda), (work), func() *int {y := 1; return &y }())
